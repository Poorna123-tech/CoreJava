...
public class Mainn {
	...
	public static void main(String[] args) {
		...
		if (n.equals("login")) {
			...
			while (!(validation.validateUserId(userId))) {
				count++;
				if (count == 3)
					return;
				else {
			...
			while (!validation.validateFirstName(firstName)) {
				count++;
				if (count == 3)
					return;
				else {
			...
			while (!(validation.validateLastName(lastName))) {
				count++;
				if (count == 3)
					return;
				else {
					System.out.println("Invalid matches. Try again");
					lastName = sc.next();
				}
			}
			...
			while (!b) {
				...
				if (count == 3)
					...
					System.out.println(
							"Should contain atleast one uppercase[A-Z], one lowercase[a-z], one special characters[@,_,&], onenumerics[0-9]");
			...
			while (!validation.validateMobileNumber(mobileNo)) {
				count++;
				if (count == 3)
					return;
				else {
			...
			if (validate.validateFirstName(a) & validate.validateLastName(b) & Validation.validateEmail(c)
					& validate.validateMobileNumber(d) & validate.validateUserId(e) & validate.validatePassword(f)) {
